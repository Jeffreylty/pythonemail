...
def send_email(smtpHost,port, sendAddr, password, recipientAddrs, subject='', content='', file_path):
    ...
    part.add_header('Content-Disposition', 'attachment', filename="chromedriver")#给附件重命名,一般和原文件名一样,改错了可能无法打开.
    msg.attach(part)
...
if __name__ == "__main__":
    try:
        ...
        port = 465 #端口号
        sendAddr ='2684199641@qq.com'#发送方地址
        password = input("请输入邮箱授权码：")#手动输入授权码更安全.授权码的获取:打开qq邮箱->设置->账户->开启IMAP/SMTP服务->发送短信->授权码
        file_path =  input("请输入文件地址: ")
        recipientAddrs = 'liutianyi20@huawei.com'#接收方可以是多个账户, 用分号分开,send_email()函数中手动设置
        subject='python test'#主题
        content='Hello world'#正文内容
        send_email(smtpHost, port, sendAddr, password, recipientAddrs, subject, content, file_path)#调用函数
